...
(() => {
	...
	if (!globalThis.fs) {
		...
		globalThis.fs = {
			...
			futimes(fd, atime, mtime, callback) { callback(enosys()); },
			...
			rename(oldPath, newPath, callback) { callback(enosys()); },
			rmdir(path, callback) { callback(enosys()); },
			stat(path, callback) { callback(enosys()); },
			symlink(target, path, callback) { callback(enosys()); },
	...
	globalThis.Go = class {
		constructor() {
			...
			const goImports = {
					// Go's missing _timeih (runtime.nanotime)
					"runtime.wasmExit": (sp) => {
						sp >>>= 0;
						...
						this._scheduledTimeouts.set(id, setTimeout(
							() => {
								...
								while (this._scheduledTimeouts.has(id)) {
									console.warn("scheduleTimeoutEvent: continuous execution");
									this._resume();
								}
							},
							getInt64(sp + 8),
					...
					"syscall/js.stringVal": (sp) => {
						sp >>>= 0;
						const value = loadString(sp + 8);
						storeValue(sp + 24, value);
					},
					"syscall/js.valueGet": (sp) => {
						sp >>>= 0;
						const result = Reflect.get(loadValue(sp + 8), loadString(sp + 16));
						sp = this._inst.exports.getsp() >>> 0; // sp might have changed
		...
		async run(instance) {
			...
			argvPtrs.forEach((ptr) => {
				new DataView(this._inst.exports.mem.buffer).setUint32(offset, ptr, true);
				new DataView(this._inst.exports.mem.buffer).setUint32(offset + 4, 0, true);
				offset += 8;
			});

			if (offset >= 4096 * 10) {
				throw new Error("total length of command line arguments and environment variables exceeds limit");
			}
